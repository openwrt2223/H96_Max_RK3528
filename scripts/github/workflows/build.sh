...
df -mh
# 
./compile.sh build BOARD=rk3528-tvbox BRANCH=legacy BUILD_DESKTOP=no BUILD_MINIMAL=yes KERNEL_CONFIGURE=no KERNEL_GIT=shallow RELEASE=jammy
# 
...
if [ -f "output/images/Armbian-unofficial_26.02.0-trunk_Rk3528-tvbox_jammy_legacy_5.10.160_mate_desktop.img" ];then
../rclone --config ../rclone.conf mkdir odd:/dl.onkccpdt.top/storage/armbian/${date}/
../rclone --config ../rclone.conf copy -P output/images/Armbian-unofficial_26.02.0-trunk_Rk3528-tvbox_jammy_legacy_5.10.160_mate_desktop.img odd:/dl.onkccpdt.top/storage/armbian/${date}/
else
echo "Armbian-unofficial_26.02.0-trunk_Rk3528-tvbox_jammy_legacy_5.10.160_mate_desktop.img文件不存在!"
fi
